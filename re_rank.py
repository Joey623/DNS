...
def k_reciprocal(probFea, galFea, k1=20, k2=6, lambda_value=0.3, MemorySave=False, Minibatch=2000):
    ...
    if MemorySave:
        ...
        original_dist = cdist(feat, feat).astype(np.float16)  # 4104,4104
        original_dist = np.power(original_dist, 2).astype(np.float16)  # 4104 , 4104
    del feat
...
def random_walk(query_feat, gall_feat, alpha=0.5):
