...
parser.add_argument('--data_path',default='D:\\Datasets\\Re-ID\\SYSU-MM01\\',type=str, help='./test_data')
...
query_img, query_label = process_test_regdb(data_path, trial=1, modal='thermal')
gall_img, gall_label = process_test_regdb(data_path, trial=1, modal='visible')
...
for j in range(30000):
    if j % 10 == 0:
        print(j)
        result = scipy.io.loadmat('feat.mat')
        ...
        #####################################################################
        #Show result
        def imshow(path, color='None', title=None):
            """Imshow for Tensor."""
            # im = plt.imread(path)
            ...
            if color != 'None':
                ImageOps.expand(im, border=10, fill=color)
            plt.imshow(im)
        ...
        for i in range(10):
            ...
            if int(label) == int(query_label):
                color = 'green'
                imshow(img_path, color)
                ax.set_title('%d'%(i+1), color=color)
                print(img_path, 'True')
            else:
                color = 'red'
                imshow(img_path, color)
