...
# each changes
parser.add_argument('--dataset', default='mask1k', help='dataset name: mask1k')
parser.add_argument('--lr', default=0.0009, type=float, help='learning rate, 0.00035 for adam, 0.0007for adamw')
parser.add_argument('--optim', default='adamw', type=str, help='adamw or sgd')
...
if dataset == 'mask1k':
    ...
    log_path = args.log_path + 'mask1k_log/'
    test_mode = [1, 2]

suffix = dataset
suffix = suffix + '_style{}_{}_p{}_n{}_lr_{}_seed_{}'.format(args.train_style, args.optim, args.batch_size,
                                                             args.num_pos, args.lr, args.seed)
...
def adjust_lr(optimizer, batch_idx, lrs):
    for idx, param_group in enumerate(optimizer.param_groups):
        param_group['lr'] = lrs[batch_idx - 1] * lr_multipliers[idx]
        ...
        if len(args.test_style) == 1:
            with torch.no_grad():
                for batch_idx, (input, label) in enumerate(query_loader):
                    ...
                    if args.tta:
                        feat_tta, feat_att_tta = net(torch.flip(input, dims=[3]), torch.flip(input, dims=[3]),
                                                     test_mode[0])
                        feat += feat_tta
                        feat_att += feat_att_tta
...
for epoch in range(start_epoch, args.max_epoch):
    ...
    # identity sampler
    sampler = IdentitySampler(trainset.train_color_label, trainset.train_sketch_label, color_pos, sketch_pos,
